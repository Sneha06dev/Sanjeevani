...
const Signup = (props) => {
  const [data, setData] = useState({
    ...
    password: "",
    cPassword: "",
    role: "0", // default role is set to 'user'
    error: false,
    loading: false,
    success: false,
  });
  ...
  const formSubmit = async () => {
    ...
    console.log("Submitting with role: ", data.role);
    try {
      let responseData = await signupReq({
        ...
        role: data.role, // sending role to the backend
      });
  ...
  return (
    <Fragment>
      ...
      <form className="space-y-4">
        ...
        <div className="flex flex-col">
          <label htmlFor="role">
            Role<span className="text-sm text-gray-600 ml-1">*</span>
          </label>
          <select
  onChange={(e) =>
    setData({
      ...
      success: false,
      error: {},
      role: e.target.value, // Ensure this correctly sets 0 or 1
    })
  }
  value={data.role}
  id="role"
