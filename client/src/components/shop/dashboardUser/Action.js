...
export const updatePersonalInformationAction = async (dispatch, fData) => {
  ...
  try {
    ...
    // Check if responseData and responseData.success exist
    if (responseData && responseData.success) {
      dispatch({ type: "loading", payload: false });
      // Optionally, you can dispatch a success action if needed
      dispatch({ type: "updateSuccess", payload: responseData.message });
      // Fetch updated data or handle the success state
      fetchData(dispatch);
    } else {
      // Handle failure case if responseData does not have success
      dispatch({ type: "loading", payload: false });
      // Optionally, dispatch an action for failure
      dispatch({ type: "updateFailure", payload: responseData.message });
    }
  } catch (error) {
    ...
    dispatch({ type: "loading", payload: false });
    // Optionally, dispatch an action for failure
    dispatch({ type: "updateFailure", payload: "An error occurred" });
  }
};
